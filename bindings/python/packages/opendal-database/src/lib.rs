...
fn _opendal_database(py: Python, m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    py.import("sys")?
        .getattr("modules")?
        .set_item("opendal_database._opendal_database.layers", layers_module)?;
    ...
    py.import("sys")?
        .getattr("modules")?
        .set_item("opendal_database._opendal_database.exceptions", exception_module)?;
    Ok(())
}
