...
fn _opendal_core(py: Python, m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    py.import("sys")?
        .getattr("modules")?
        .set_item("opendal_core._opendal_core.layers", layers_module)?;
    ...
    py.import("sys")?
        .getattr("modules")?
        .set_item("opendal_core._opendal_core.exceptions", exception_module)?;
    Ok(())
}