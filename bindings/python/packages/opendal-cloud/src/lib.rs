...
fn _opendal_cloud(py: Python, m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    py.import("sys")?
        .getattr("modules")?
        .set_item("opendal_cloud._opendal_cloud.layers", layers_module)?;
    ...
    py.import("sys")?
        .getattr("modules")?
        .set_item("opendal_cloud._opendal_cloud.exceptions", exception_module)?;
    Ok(())
}
