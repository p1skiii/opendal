...
// Import all shared types and functions
pub use opendal_python_shared::*;

#[pymodule(gil_used = false)]
fn _opendal_advanced(py: Python, m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    py.import("sys")?
        .getattr("modules")?
        .set_item("opendal_advanced._opendal_advanced.layers", layers_module)?;
    ...
    py.import("sys")?
        .getattr("modules")?
        .set_item("opendal_advanced._opendal_advanced.exceptions", exception_module)?;
    Ok(())
}
